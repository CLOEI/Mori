...
export const LUA_DOC_SECTIONS: DocSection[] = [
  {
    ...
    entries: [
      ...
      { name: 'getWorld', signature: 'getWorld() -> World | nil', description: 'Returns the current world snapshot, or nil if not in a world.' },
...
listenEvents(30)`,
  },
  {
    ...
    entries: [
      { name: 'getWorld', signature: 'getWorld() -> World | nil', description: 'Returns a snapshot of the current world, or nil if not in one.' },
  ...
  },
  {
    id: 'world',
    title: 'World',
    summary: 'A snapshot of the world at the time getWorld() was called.',
    entries: [
      { name: 'getTile', signature: 'getTile(x: number, y: number) -> Tile | nil', description: 'Returns the tile at the given position.' },
      { name: 'getTiles', signature: 'getTiles() -> table<Tile>', description: 'Returns all tiles.' },
      { name: 'getObject', signature: 'getObject(oid: number) -> NetObject | nil', description: 'Returns the dropped object with the given OID.' },
      { name: 'getObjects', signature: 'getObjects() -> table<NetObject>', description: 'Returns all dropped objects.' },
