...
struct LuaInventory(Inventory);
struct LuaInventoryItem(InventoryItem);
struct LuaPlayer(Player);
struct LuaTile(Tile);
struct LuaNetObject(WorldObject);
struct LuaItemInfo(ItemInfo);
struct LuaGameUpdatePacket(GameUpdatePacket);
struct LuaVariant(Variant);
struct LuaVariantList(VariantList);
struct LuaLogin { mac: String }
struct LuaConsole(Arc<RwLock<BotState>>);
...
fn lua_http_options(opts: Option<LuaTable>) -> LuaResult<LuaHttpOptions> {
    ...
    let body = match opts.get::<LuaValue>("body")? {
        ...
        _ => return Err(LuaError::runtime("http options.body must be a string, number, or boolean")),
    };
    ...
    Ok(LuaHttpOptions { headers, body, timeout_ms })
}

fn make_http_request<'lua>(
    lua: &'lua Lua,
    method: &str,
    url: String,
