...
pub fn lock_the_world(bot: &Arc<Bot>) {
    ...
    bot::send_packet(&bot, EPacketType::NetMessageGenericText, "ftue_start_popup_close`".to_string());
    thread::sleep(std::time::Duration::from_millis(1000));
    bot::place(&bot, 0, -1, 9640);
        ...
        for tile in tiles.iter() {
            if tile.foreground_item_id == DIRT {
                ...
                while {
                    ...
                    bot::find_path(&bot, tile.x, tile.y - 1);
        ...
        for tile in tiles.iter() {
            if tile.foreground_item_id == DIRT {
                ...
                while {
                    ...
                    bot::find_path(&bot, tile.x, tile.y - 1);
                    thread::sleep(std::time::Duration::from_millis(100));
                    bot::place(&bot, 0, 0, DIRT_SEEDS as u32);
                    thread::sleep(std::time::Duration::from_millis(250));
                }
            }
        }
...
pub fn splice_rock_and_cbg_seed(bot: &Arc<Bot>) {
    ...
    thread::sleep(std::time::Duration::from_millis(100));
    bot::place(&bot, 1, 0, CAVE_BACKGROUND_SEED as u32);
    thread::sleep(std::time::Duration::from_millis(100));
    bot::place(&bot, 1, 0, ROCK_SEED as u32);
    thread::sleep(std::time::Duration::from_millis(100));
}
...
pub fn place_sign_in_world(bot: &Arc<Bot>) {
    ...
    loop {
        ...
        if is_harvestable {
            bot::find_path(&bot, sign_tree_tile.x, sign_tree_tile.y);
            thread::sleep(std::time::Duration::from_millis(100));
            bot::punch(&bot, 0, 0);
            thread::sleep(std::time::Duration::from_millis(250));
            break;
...
pub fn wrench_sign(bot: &Arc<Bot>) {
    ...
    if sign_tile.is_some() {
        ...
        bot::find_path(&bot, sign_tile.x, sign_tile.y);
        thread::sleep(std::time::Duration::from_millis(100));
        bot::wrench(&bot, 0, 0);
        thread::sleep(std::time::Duration::from_millis(1000));
        bot::send_packet(&bot, EPacketType::NetMessageGenericText, format!("action|dialog_return\ndialog_name|sign_edit\ntilex|{}|\ntiley|{}|\nsign_text|CLOEI\n", sign_tile.x, sign_tile.y).to_string());
        thread::sleep(std::time::Duration::from_millis(1000));
        while {
            let world = &bot.world.read().unwrap();
...
pub fn harvest_and_place_wood_block(bot: &Arc<Bot>) {
    ...
    bot::find_path(&bot, 1, 7);
    for i in 0..10 {
        bot::walk(&bot, i, 0, false);
        thread::sleep(std::time::Duration::from_millis(100));
        bot::place(&bot, 0, -1, WOOD_BLOCK as u32);
        thread::sleep(std::time::Duration::from_millis(250));
    }
}
